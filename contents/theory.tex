...
\section{Top Quarks and $t\bar{t}t\bar{t}$ Production}
\label{sec:four-tops}
...
\subsection{$t\bar{t}t\bar{t}$ Production in the Standard Model}
...
\end{tikzpicture}
\end{subfigure}
\caption{Illustration of the Standard Model $t\bar{t}t\bar{t}$ production The mediator connecting two top quarks can be a gluon, a neutral electroweak gauge boson ($\gamma$ or $Z$), or a Higgs boson.
.}
...
\subsection{BSM Contributions to $t\bar{t}t\bar{t}$ Production}

The $t\bar{t}t\bar{t}$ final state is particularly sensitive to new physics scenarios that feature enhanced couplings to the top quark. 
...
\paragraph{$t\bar{t}t\bar{t}$ production in 2HDM.}
...
\paragraph{$t\bar{t}t\bar{t}$ production via $S_{8}$.}
...
The pair production process therefore leads to a $t\bar{t}t\bar{t}$ final state
...
$t\bar{t}t\bar{t}$ final state arises from $S_{8}$ pair production followed by the
...
\caption{BSM contributions to $t\bar{t}t\bar{t}$ production: (left) 2HDM heavy scalar \(H/A\); (right) $S_{8}$ pair decay.}
